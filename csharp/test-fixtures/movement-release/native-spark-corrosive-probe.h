...
// Lava/acid travel in all directions with/without Gravity Suit, including damage.
// Health starts at 999; capture ends at collision or energy termination, cheats off.
int DiagnosticSparkCorrosive(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"direction,left,liquidcase,frame,input,pose,shine,windup,x,y,health,crash,subhealth\n");
  for(int medium=0;medium<3;medium++) for(int left=0;left<2;left++) for(int offset=0;offset<4;offset++) {
    ...
    fx_y_pos=0xffff; lava_acid_y_pos=8;
    fx_liquid_options=0x80; fx_type=offset<2?2:4; liquid_physics_type=2;
    ...
    for(int frame=0;frame<1200;frame++) {
      ...
      fprintf(f,"%d,%d,%d,%d,%04X,%04X,%04X,%04X,%04X%04X,%04X%04X,%04X,%d,%04X\n",
        medium,left,offset,frame,input,samus_pose,samus_shine_timer,timer_for_shinesparks_startstop,
        samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,samus_health,samus_movement_handler==0xd346,samus_subunit_health);
