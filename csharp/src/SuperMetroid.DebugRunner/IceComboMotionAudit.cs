...
internal static class IceComboMotionAudit
{
    public static int Run(string rom, string trace)
    {
            ...
            .GroupBy(row => string.Join(',', row[..2])))
        {
            ...
            var samus = new SamusState { XPosition = 128, YPosition = 128, PowerBombs = 2,
                EquippedBeams = 0x1002, SelectedHudItem = 3,
            ...
            foreach (var row in group)
            {
                ...
                for (int i = 3; i >= 0; i--)
                    ...
                    {
                        ushort sound = projectiles.StepIceCombo(bus, samus, projectiles.Slots[i], shared, 0, 0);
                ...
                string actual = $"{projectiles.ProjectileCounter:X4},{shared.CooldownTimer:X4},{projectiles.FlareCounter:X4},{sounds}," +
                    string.Join('/', projectiles.Slots.Take(4).Select(slot => !slot.IsActive ? "0" :
                        $"{slot.XPosition:X4}{slot.YPosition:X4}{(slot.PreInstruction == SamusProjectilePreInstruction.IceCombo ? SamusComboRomData.Ice : SamusComboRomData.IceOutward):X4}" +
                        $"{slot.Variable:X4}{(ushort)slot.XVelocity:X4}{(ushort)slot.YVelocity:X4}{slot.TrailTimer:X4}"));
                if (actual != string.Join(',', row[3..]))
                    if (failures++ < 8) Console.WriteLine($"ICE {group.Key}/{frame}: {actual} != {string.Join(',', row[3..])}");
                frames++;
            }
        }
        if (frames != 3840) throw new InvalidDataException("Incomplete Ice matrix.");
        Console.WriteLine($"Ice combo motion: {frames} frames, {failures} mismatches.");
