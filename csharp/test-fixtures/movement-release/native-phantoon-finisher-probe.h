...
int DiagnosticPhantoonFinisher(const char *rom, const char *output) {
  ...
  static const int fires[] = {1720,1721};
  static const int healths[] = {1000,1001};
  for (int test = 0; test < 4; test++) {
    int fire = fires[test % 2], initialHealth = healths[test / 2];
    ...
    for (int frame = -2; frame < 1850; frame++) {
      ...
      if (frame >= 1680) {
        input = frame == 1680 ? 0x200 : 0;
        if ((frame <= 1710 && (frame - 1680) % 10 == 0) || frame == fire) input |= 0x40;
        if (frame == 1719) input |= 0x2000;
      }
