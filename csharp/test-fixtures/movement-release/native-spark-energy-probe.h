...
// Dry travel through energy termination or room collision in all three directions.
// Starting health straddles the native cutoff; no invincibility override is applied.
int DiagnosticSparkEnergy(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"direction,left,energycase,frame,input,pose,shine,windup,x,y,health,crash\n");
  for(int medium=0;medium<3;medium++) for(int left=0;left<2;left++) for(int offset=0;offset<6;offset++) {
    ...
    fx_y_pos=0xffff; lava_acid_y_pos=0xffff;
    fx_liquid_options=0x80; fx_type=0; liquid_physics_type=0;
    equipped_items=0x2000; enable_horiz_slope_coll=3;
    const int energies[]={1,28,29,30,31,99}; samus_health=energies[offset]; samus_max_health=99;
