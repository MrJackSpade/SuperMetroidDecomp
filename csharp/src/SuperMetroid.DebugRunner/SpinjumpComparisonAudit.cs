    ...
    public static int Run(string rom, string trace)
    {
        ...
        for (int delay = 0; delay <= 8; delay++)
        {
            ...
            for (int frame = -24; frame < 40; frame++)
            {
                ...
                samus.InputLocked = scenario == 2 && frame >= -1 && frame < 0;
                ushort input = row == null ? (ushort)(samus.InputLocked ? 0x80 : 0) : ushort.Parse(row[5], NumberStyles.HexNumber);
                    ...
                    (byte)samus.ReadMovementType(bus) != movement)
                {
                    if (mismatches++ < 24) Console.WriteLine($"SPIN water={water} left={left} scenario={scenario} delay={delay} frame={frame}: " +
                        ...
                        $"movement={(byte)samus.ReadMovementType(bus):X2}/{movement:X2} (managed/native)");
                }
            }
        }
        Console.WriteLine($"Spinjump comparison: {sample} frames, {mismatches} position/pose/movement mismatches.");
