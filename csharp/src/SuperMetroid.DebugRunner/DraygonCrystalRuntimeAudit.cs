...
{
    public static int Run(string rom, string trace, bool xrayCancellation = false, bool recharge = false, bool lifetime = false, bool repeat = false, bool sand = false, bool beam = false, bool save = false)
    {
        ...
        if (Convert.ToHexString(SHA256.HashData(bus.Rom)) != "12B77C4BC9C1832CEE8881244659065EE1D84C70C3D29E6EAF92E6798CC2CA72" ||
            Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(text))) != (save
                ? "A8FEC4FF65CD6ADBD5CFA44EFEFAFF03A5AC18F26CF9E465BB4D08246480CB87" : beam
        ...
        if (rows.Length != (sand ? 11232 : repeat ? 3200 : lifetime ? 4000 : recharge ? 5600 : xrayCancellation || beam || save ? 1404 : 6880) || rows.Any(row => row.Length != (save ? 31 : sand ? 30 : lifetime || beam ? 29 : 27)))
        ...
        foreach (var group in rows.GroupBy(row => $"{row[0]},{row[1]},{row[2]}"))
        {
            ...
            foreach (var row in group)
            {
                ...
                if (save && frame == 350) FlashSaveReloadProbe.Run(bus, runtime, row);
                if (actual != expected)
                {
                    mismatches++;
