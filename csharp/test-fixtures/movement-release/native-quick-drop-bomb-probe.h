...
// #456: bomb and Power Bomb block dispatch during a falling turn.
int DiagnosticQuickDropBomb(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,power,permanent,delay,blast,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,facing,yspeed,ydir,chargecounter,block\n");
  for (int left = 0; left < 2; left++)
  for (int power = 0; power < 2; power++)
  for (int permanent = 0; permanent < 2; permanent++)
  ...
  for (int blast = 0; blast <= 3; blast++) {
    ...
    plm_flag = 0x8000;
    for (int x = 62; x <= 66; x++) {
      level_data[16 * 144 + x] = 0xf000;
      BTS[16 * 144 + x] = permanent ? 4 : 0;
    }
    ...
    for (int frame = 0; frame < 60; frame++) {
      ...
      if (frame == blast) {
        projectile_index = 10; projectile_type[5] = power ? 0x0300 : 0x0500;
        for (int x = 62; x <= 66; x++)
          ProbeRunBoundedRegisters(0x94a052, 0, 2 * (16 * 144 + x), 0);
        projectile_type[5] = 0;
      }
      ...
      fprintf(f, "%d,%d,%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%02X,%04X%04X,%04X,%04X,%04X\n",
        left,power,permanent,delay,blast,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,
