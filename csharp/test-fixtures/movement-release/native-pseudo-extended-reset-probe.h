...
// #421: extended enemy collision entry, with non-overlapping geometry so no touch
// callback runs. This isolates invulnerability reset from subsequent contact damage.
int DiagnosticPseudoExtendedReset(const char *rom, const char *output) {
  ...
  for(int noop=0;noop<3;noop++) for(int inv=0;inv<2;inv++) {
    cpu_reset(g_snes->cpu); memset(g_ram,0,sizeof(g_ram));
    g_snes->cpu->e=false; g_snes->cpu->sp=0x1ff0; g_snes->cpu->dp=0;
    memset(def,0,sizeof(*def)); def->touch_ai=noop==2?0x804b:noop?0x804c:0xa477;
    EnemyData *enemy=gEnemyData(0); enemy->enemy_ptr=0xf000; enemy->bank=0xa3;
    enemy->spritemap_pointer=map?1:0;
