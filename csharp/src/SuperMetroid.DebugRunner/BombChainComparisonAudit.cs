...
{
    public static int Run(string rom, string trace, bool repeated = false)
    {
        var bus = SuperMetroidAddressSpace.LoadRetailRom(rom);
        var rows = File.ReadLines(trace).Skip(1).Select(line => line.Split(',')).ToArray();
        if (rows.Length != (repeated ? 64800 : 12960) || rows.Any(row => row.Length != 49))
        ...
        foreach (var group in rows.GroupBy(row => string.Join(',', row[..4])))
        {
            ...
            if (seed[0] is not ("0" or "1") || seed[1] is not ("0" or "1") || travel is < 0 or > 2 ||
                (repeated ? spacing is < 48 or > 56 : spacing is not (0 or 40 or 44 or 48 or 52 or 56)))
            ...
            ushort previousBombDirection = 0;
            ...
            foreach (var row in group)
            {
                ...
                ushort expectedInput = (repeated ? frame % spacing == 0 : frame == 0 || spacing != 0 && (frame == spacing || frame == spacing * 2)) ? (ushort)0x40 : (ushort)0;
                ...
                if ((samus.BombJumpDirection & 0xff00) != 0 && previousBombDirection == 0)
                {
                    launches++;
                    launchHeights.Add(samus.Kinematics.YFixed);
                }
                previousBombDirection = samus.BombJumpDirection;
        ...
        if (cases != (repeated ? 108 : 72)) throw new InvalidDataException("Incomplete bomb-chain matrix.");
