...
// Stand/crouch launch attempts with held Shoot/Aim Up/Aim Down combinations.
// Crouch is entered through Down input before storage, not by replacing the pose.
int DiagnosticSparkGroundRestrictions(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"medium,left,offset,frame,input,pose,shine,windup,x,y\n");
  for(int medium=0;medium<3;medium++) for(int left=0;left<2;left++) for(int offset=0;offset<16;offset++) {
    ...
    for(int frame=0;frame<64;frame++) {
      ...
      if(offset>=8 && frame>=8 && frame<12) input|=0x400;
      if(frame>=14) { const uint16 held[]={0,0x40,0x20,0x60,0x10,0x50,0x30,0x70}; input|=held[offset&7]; }
