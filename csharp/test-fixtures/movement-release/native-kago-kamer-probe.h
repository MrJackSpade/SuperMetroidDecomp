...
// #455: two Kamer families, approach and rider geometries, controller transitions.
int DiagnosticKagoKamer(const char *rom, const char *output) {
  ...
  for (int delay = 0; delay < 12; delay++) {
    ...
    for (int x = 0; x < 144; x++) level_data[(geometry ? 48 : 32) * 144 + x] = 0x8000;
    fx_y_pos = lava_acid_y_pos = 0xffff;
    equipped_items = 4; enable_horiz_slope_coll = 3;
    samus_health = samus_max_health = 999;
    samus_x_pos = samus_prev_x_pos = 1024; samus_y_pos = samus_prev_y_pos = (geometry ? 483 : 491) + (pattern == 2 ? 14 : 0);
    ...
    uint16 definition=actor ? 0xd83f : 0xd5ff;
    ...
    enemy->x_pos=actor && !geometry ? 992 : 1024;
    enemy->y_pos=geometry ? 512 : (actor ? 488 : 448);
    enemy->properties=0xa800; enemy->layer=header->layer;
    enemy->instruction_timer=1;
    enemy->current_instruction=actor ? 0 : (geometry ? 0x10 : 0x110);
    enemy->extra_properties=actor ? 0 : 0x0404;
    enemy->parameter_1=actor ? 1 : 0x8000;
    enemy->parameter_2=actor ? 0x2810 : 8;
