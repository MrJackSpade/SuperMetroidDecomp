...
int DiagnosticPhantoonPattern(const char *rom, const char *output) {
  ...
  for (int start = 1640; start <= 1700; start += 10)
  for (int cadence = 10; cadence <= 11; cadence++)
  for (int movement = 3; movement <= 6; movement++) {
    ...
    uint16 previous = 0;
    for (int frame = -2; frame < 1850; frame++) {
      ...
      if (frame == 1565 || frame == 1575) input |= 0x40;
      if (frame >= start) {
        int elapsed = frame - start;
        bool shot = elapsed == 0 || elapsed == 10 || elapsed == 40 || elapsed == 50 || (elapsed >= 80 && (elapsed - 80) % cadence == 0);
        bool walk = movement == 3 || (movement == 4 && elapsed >= 80) || (movement == 5 && elapsed % 20 < 10);
        input = walk || elapsed == 0 ? 0x200 : 0;
        if (shot) input |= 0x40;
      }
