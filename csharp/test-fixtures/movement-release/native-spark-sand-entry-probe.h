...
// Launch outside sand, then enter surface/submerging sand in each direction.
// Ceiling and side patches allow both suitless and Gravity sparks to launch first.
int DiagnosticSparkSandEntry(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"direction,left,sandcase,frame,input,pose,shine,windup,x,y,health,crash\n");
  for(int medium=0;medium<3;medium++) for(int left=0;left<2;left++) for(int offset=0;offset<4;offset++) {
    ...
    interactive_enemy_indexes[0]=0xffff; area_index=4; plm_flag=0x8000;
    for(int x=0;x<16;x++) level_data[x]=level_data[16*16+x]=0x8000;
    for(int y=0;y<16;y++) level_data[y*16]=level_data[y*16+15]=0x8000;
    for(int y=8;y<16;y++) for(int x=1;x<15;x++) {
      if(y>=13 && (left ? x>4 : x<11)) continue;
      level_data[y*16+x]=0x3000; BTS[y*16+x]=offset<2?0x82:0x83;
    }
