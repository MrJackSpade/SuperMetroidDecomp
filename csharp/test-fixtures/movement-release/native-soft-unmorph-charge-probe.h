// #471: soft-unmorph preparation followed by selected-X-ray charge carry and mockball.
int DiagnosticSoftUnmorphCharge(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,downFrame,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,yspeed,ydir,charge,spread,bombs,type,pointer0,x0,y0,vx0,vy0,bounce,hud,frozen,beams,height,up\n");
  for (int height = 201; height <= 203; height++) for (int up = 19; up <= 21; up++)
  for (int left = 0; left < 2; left++) for (int downFrame = 224; downFrame <= 241; downFrame++) {
    ...
    for (int frame = 0; frame < 320; frame++) {
      ...
      if (frame == 60) input |= 0x2000;
      if (frame >= 61 && frame <= 125 || frame >= downFrame + 1) input |= 0x40;
      if (frame >= 126 && frame <= downFrame + 1) input |= 0x8000;
      if (frame >= 130 && (frame < downFrame - 2 || frame > downFrame)) input |= forward;
      if (frame >= 145) input |= 0x80;
      if (frame == downFrame - 2 || frame >= downFrame && frame < 300) input |= 0x400;
      ...
      fprintf(f, "%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%04X%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%d,%d\n",
        ...
        projectile_x_pos[5],projectile_y_pos[5],projectile_bomb_x_speed[5],projectile_bomb_y_speed[5],used_for_ball_bounce_on_landing,hud_item_index,time_is_frozen_flag,projectile_counter,height,up);
      // Adjacent ordinary landings are preparation controls; stop before actually
      // activating a scope, whose full HDMA lifecycle belongs to a different fixture.
      if (frame == 125 && !(samus_y_speed || samus_y_subspeed)) break;
    }
  }
  fclose(f); return 0;
}
