...
int DiagnosticXPlasmaPhantoon(const char *rom, const char *output) {
  ...
  fprintf(f, "phase,primed,type,stage,health,invincibility,flash,function,timer,properties,accum,started,reaction,direction,x,y,instruction\n");
  ...
  for (int kind = 0; kind < 3; kind++) {
    ...
    body->base.current_instruction = 0xcc41;
    ...
    for (int stage = 0; stage < 22; stage++) {
      ...
      fprintf(f, "%u,%d,%u,%d,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
        ...
        body->base.x_pos, body->base.y_pos, body->base.current_instruction);
    }
  }
  fclose(f); return 0;
}
