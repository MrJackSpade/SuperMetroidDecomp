// #471: build charge, jump into a solid wall, walljump and attempt a charged soft morph.
int DiagnosticChargedWalljump(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,downFrame,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,yspeed,ydir,charge,spread,bombs,type,pointer0,x0,y0,vx0,vy0,bounce\n");
  for (int left = 0; left < 2; left++) for (int downFrame = 150; downFrame <= 210; downFrame += 5) {
