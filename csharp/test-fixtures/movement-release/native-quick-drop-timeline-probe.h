...
// #456: real controller entry/completion around a temporary ceiling.
int DiagnosticQuickDropTimeline(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,delay,remove,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,facing,yspeed,ydir,chargecounter\n");
  ...
  for (int remove = 0; remove <= 13; remove++) {
    ...
    for (int x = 0; x < 144; x++) level_data[12 * 144 + x] = 0x8000;
    ...
    for (int frame = 0; frame < 80; frame++) {
      ...
      if (remove < 13 && frame == 8 + remove)
        for (int x = 0; x < 144; x++) level_data[12 * 144 + x] = 0;
      uint16 input = frame >= 8 && frame < 50 ? 0x80 : 0;
      if (frame >= 8 + delay && frame < 50) input |= left ? 0x100 : 0x200;
      ...
      fprintf(f, "%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%02X,%04X%04X,%04X,%04X\n",
        left,delay,remove,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,
