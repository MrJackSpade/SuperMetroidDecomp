...
// #463: real item detector side effects through the observational wall-check entry.
int DiagnosticRemoteItem(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,gap,blocker,fraction,distance,x,xsub,y,ysub,collision,amount,trigger,owner\n");
  ...
  for (int distance=1;distance<=9;distance++) {
    ...
    *(uint16 *)(g_ram+0x14)=0;
    ...
    fprintf(f,"%d,%d,%d,%d,%d,%04X,%04X,%04X,%04X,%04X,%04X%04X,%04X,%04X\n",
      ...
      samus_collision_flag,*(uint16 *)(g_ram+0x12),*(uint16 *)(g_ram+0x14),plm_timers[39],plm_header_ptr[39]);
  }
  fclose(f); return 0;
}
