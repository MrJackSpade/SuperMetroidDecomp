...
The C# room viewer and translated subsystem runner work now: the frame tab combines recognizable ROM-composed Landing Site terrain with live HUD, OAM, Samus animation, collision, camera, and minimap state. The deliberately exposed all-live PPU toggle renders cartridge-backed BG1, bank-$88's circular/HDMA-scrolled BG2, and exact BG3 HUD while the remaining library-background behavior is translated. Grounded Samus can stand and run in both directions using poses `$01/$02/$09/$0A`; reversals execute the cartridge's `$25/$26` turn poses, mode-one old-direction momentum, and animation command `$F8`. Bank-$94 solid/square/non-square slope collision, bank-$80/$90 camera tracking, room scroll zones, BG streaming, 32x64 BGSC addressing, and the Landing Site door/library-background selection are translated and verified. The viewer supplies an explicitly host-selected spawn because the landing cinematic does not define a normal-gameplay Samus spawn. Jumping/falling, special block dispatchers, PLMs, enemies, effects, and cutscene actors remain incremental work. The native C build remains the complete playable desktop reference.
