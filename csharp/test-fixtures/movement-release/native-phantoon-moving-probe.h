...
int DiagnosticPhantoonMoving(const char *rom, const char *output) {
  ...
  const int movements[] = {12,12,13,13};
  const int cadences[] = {10,11,10,11};
  for (int test = 0; test < 4; test++) {
    int start = 1680, cadence = cadences[test], movement = movements[test];
    ...
    for (int frame = -2; frame < 1850; frame++) {
      ...
      if (frame >= start) {
        input = frame == start || frame - start >= 20 + (movement - 11) * 5 ? 0x200 : 0;
