...
internal static class WallJumpComparisonAudit
{
    public static int Run(string rom, string trace)
    {
        ...
        if (rows.Length != 780) throw new InvalidDataException("Expected 26 cases of 30 frames.");
        ...
        for (int delay = 0; delay <= 12; delay++)
        {
            ...
            for (int frame = 0; frame < 30; frame++)
            {
                ...
                if (actual != expected && mismatches++ < 16)
                    Console.WriteLine($"WALL left={left} delay={delay} frame={frame}: {actual} != {expected}");
                if (firstWallJump < 0 && samus.ReadMovementType(bus) == SamusMovementType.WallJumping) firstWallJump = frame;
            }
            Console.WriteLine($"WINDOW left={left} delay={delay} firstWallJump={firstWallJump}");
        }
