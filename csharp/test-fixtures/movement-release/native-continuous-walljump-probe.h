// #445/#471: charged real-input continuous walljump over a platform, then soft morph.
int DiagnosticContinuousWalljump(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,downFrame,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,yspeed,ydir,charge,spread,bombs,type,pointer0,x0,y0,vx0,vy0,bounce,jump,skipRelease\n");
  for (int skipRelease = 0; skipRelease < 2; skipRelease++)
  for (int left = 0; left < 2; left++) for (int jump = 143; jump <= 147; jump++)
  for (int downFrame = 230; downFrame <= 250; downFrame++) {
    ...
    samus_x_pos = samus_prev_x_pos = left ? 1281 : 1024; samus_y_pos = samus_prev_y_pos = 235;
    ...
    for (int frame = 0; frame < 320; frame++) {
      ...
      if (frame >= 30 && (skipRelease || frame != jump - 2)) input |= forward;
      if (frame >= 30 && frame < 70) input |= 0x8000;
      if (frame >= 70 && frame < jump - 1 || frame >= jump) input |= 0x80;
      if (frame >= downFrame) input |= 0x40 | 0x400;
      if (frame >= 300) input &= ~0x400;
      ...
      fprintf(f, "%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%04X%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%d,%d\n",
        ...
        projectile_x_pos[5],projectile_y_pos[5],projectile_bomb_x_speed[5],projectile_bomb_y_speed[5],used_for_ball_bounce_on_landing,jump,skipRelease);
    }
  }
  fclose(f); return 0;
}
