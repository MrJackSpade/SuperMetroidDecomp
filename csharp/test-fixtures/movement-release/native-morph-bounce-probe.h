...
int DiagnosticMorphBounceVariant(const char *rom, const char *output, int timing) {
  ...
  for (int speed = 0; speed < (timing ? 9 : 8); speed++)
  for (int carry = 0; carry < (timing ? 4 : 8); carry++)
  for (int held = 0; held < 2; held++) {
    cpu_reset(g_snes->cpu); memset(g_ram, 0, sizeof(g_ram));
    g_snes->cpu->e = false; g_snes->cpu->sp = 0x1ff0; g_snes->cpu->dp = 0;
    int grounded = timing && speed == 8;
    int width = timing ? 144 : 16;
    room_width_in_blocks = width; room_height_in_blocks = timing ? 80 : 32;
    room_width_in_scrolls = timing ? 9 : 1; room_height_in_scrolls = timing ? 5 : 2;
    ...
    samus_x_pos = samus_prev_x_pos = timing ? 512 : 128;
    samus_y_pos = samus_prev_y_pos = timing && !grounded ? 180 : 249;
    ...
    samus_movement_type = samus_prev_movement_type = samus_prev_movement_type2 = timing ? (grounded ? 4 : 6) : 8;
    samus_anim_frame_timer = 1; samus_x_speed_table_pointer = 0x9f55;
    samus_input_handler = 0xe913; samus_movement_handler = 0xa337;
    uint32 vertical = timing ? (grounded ? 0 : 0x18000) : speeds[speed];
    samus_y_speed = vertical >> 16; samus_y_subspeed = vertical; samus_y_dir = grounded ? 0 : 2;
    uint32 base = timing ? 0x14000 : carry < 4 ? carries[carry] : 0x14000;
