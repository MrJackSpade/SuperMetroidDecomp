...
int DiagnosticPhantoonWindow(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "start,cadence,aerial,frame,input,frozen,health,timer,flash,map,x,y,samusX,samusY,pose,hud,samusHealth,function,samusSubX,samusSubY");
  ...
  const int starts[] = {1680,1680,1710,1710};
  const int cadences[] = {10,11,9,10};
  for (int test = 0; test < 4; test++) {
    int start = starts[test], cadence = cadences[test], aerial = test >= 2;
    ...
    for (int frame = -2; frame < 1850; frame++) {
      ...
      if (frame >= start) {
        input = frame == start ? 0x200 : 0;
        if ((frame - start) % cadence == 0) input |= 0x40;
        if (aerial && frame < start + 20) input |= 0x80;
      }
      ...
      fprintf(f, "%d,%d,%d,%d,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
        start,cadence,aerial,frame,input,time_is_frozen_flag,body->health,body->invincibility_timer,
