...
// #464: down-aim/spin expansion under a ledge, with empty-space timing controls.
int DiagnosticEdgeBoost(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,family,speed,ledge,gap,delay,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,facing,yspeed,ydir,chargecounter,xradius,yradius\n");
  ...
  for (int delay = -1; delay <= 4; delay++) {
    ...
    if (ledge) level_data[31 * 144 + (left ? 63 : 64)] = 0x8000;
    fx_y_pos = lava_acid_y_pos = 0xffff;
    fx_liquid_options = 0x80;
    equipped_items = 4;
    enable_horiz_slope_coll = 3;
    samus_health = samus_max_health = 99;
    samus_x_pos = samus_prev_x_pos = left ? 1028 : 1020;
    samus_pose = samus_prev_pose = family ? (left ? 0x1a : 0x19) : (left ? 0x2e : 0x2d);
    samus_movement_type = samus_prev_movement_type2 = family ? 3 : 6;
    ProbeRunBounded(0x90ec22);
    samus_y_pos = samus_prev_y_pos = 512 + samus_y_radius + gap;
    samus_y_speed = speed == 2 ? 5 : speed; samus_y_dir = 2;
    ...
    uint16 previous = family ? 0x80 : 0x400;
    for (int frame = 0; frame < 96; frame++) {
      nmi_frame_counter_word = nmi_frame_counter_byte = frame + 2;
      uint16 input = family ? 0x80 : 0x400;
      if (delay >= 0 && frame >= delay)
        input = family ? 0x90 : (frame < delay + 4 ? (left ? 0x200 : 0x100) : 0);
      ...
      fprintf(f, "%d,%d,%d,%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%02X,%04X%04X,%04X,%04X,%04X,%04X\n",
        left,family,speed,ledge,gap,delay,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,
