...
`--mother-brain-rainbow-script` runs the translated `$A9:B8EB-$BE1A` repeat/active/final rainbow chain. It advances actor AI before the separate ordinary enemy-instruction stage, reads Mother Brain's real walk and posture commands/spritemap delays from the supplied ROM, preserves all four 257-call waits and the power-bomb gate, drains Samus/resources for exactly 300 calls, narrows and shuts down the beam, executes the forced post-beam fall plus 129-call health decision, and retains the suit/RNG finish-off branches. The 1,750-frame route reaches finish-off at 1047, waits for the forward body animation, copies four literal Baby graphics chunks through the live NMI queue on frames 1401..1404, verifies all `$800` VRAM bytes against the cartridge, requests the Baby spawn, and reaches Mother Brain's final firing hold at 1661. The spawned Baby's independent `$A9:C710+` flight/drain/heal/death actor remains explicit rather than guessed.
