...
int DiagnosticPhantoonFired(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "release,scope,plasma,frame,input,frozen,health,timer,flash,map,x,y,samusX,samusY,pose,hud,samusHealth,function,samusSubX,samusSubY");
  ...
  static const int releases[] = {1690,1692,1693,1697,1698,1700,1700,1700};
  for (int test = 0; test < 8; test++) {
    int scope = test != 6, release = releases[test], plasma = test != 7;
    ...
    equipped_beams = collected_beams = plasma ? 0x1008 : 0x1000;
    ...
    for (int frame = -2; frame < release + 400; frame++) {
      ...
      if (frame >= 1548 && frame < 1588) input |= 0x80;
      if (frame == 1565 || (frame >= release - 90 && frame < release)) input |= 0x40;
      if (frame == 1566) input |= 0x4000;
      if (frame == release + 1 || frame == release + 3) input |= 0x2000;
      int freezeStart = firstHit > release + 3 ? firstHit : release + 3;
      if (scope && firstHit >= 0 && frame > release + 3 && (frame - freezeStart - 1) % 64 < 60) input = 0x8000;
      ...
      if (health - body->health == (plasma ? 450 : 60) && firstHit < 0) firstHit = frame;
      fprintf(f, "%d,%d,%d,%d,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
        release,scope,plasma,frame,input,time_is_frozen_flag,body->health,body->invincibility_timer,
