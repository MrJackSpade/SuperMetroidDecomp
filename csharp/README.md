...
Ordinary Dash is a literal `$90:973E` route, not a viewer speed multiplier. Hold **Hold Run**
with Left/Right to add 0.1000 per type-one frame up to the no-booster 2.0000 cap. Native
momentum carries that component through `$09/$0A -> $19/$1A`, and running animation reads
its shared delay list through ROM pointer `$91:B5D1`. Equipped Speed Booster remains an
explicit boundary because its counter stages, palette, echoes, and alternate animation
sequences are separate systems. Useful breakpoints are
...
The first PNG pass is deliberately diagnostic: tile images use grayscale pixel indices, while palette files become accurate color swatches. The static room path composes real foreground/background block layers for asset inspection. The interactive runtime intentionally reuses that ROM-derived composition until the live library-background path reproduces all terrain; **Live PPU layers** makes the incomplete all-live result directly comparable. Camera buttons and initial Samus placement remain explicit host stimuli because the landing-cutscene door has no normal-gameplay spawn. The grounded viewer grants Morph Ball and Bomb item bits; click **Hold Shoot** on for one fresh X edge, then off before another placement. Square/non-square slope collision, grounded ordinary/aimed standing and crouched reversal, ordinary no-booster Dash and shared running animation, option-gated straight/aimed moonwalking and its turning jump, ordinary neutral/spin/crouch jumping, walk-off falling, ceiling collision, landing, animated/direct crouch-to-stand collision, stationary/running/crouching aiming, equal-radius aimed jumping/falling/landing, compact straight-down radius/landing collision, aerial turns, block-triggered dry wall jump, ordinary dry-air knockback/damage boost, grapple firing/persistent-block acquisition/all thirty standing-crouching-airborne connection records/terrain reflection/collision kick/exact locked and wallgrab angles/grace-window wall jump/dropped-pose selection/release/beam rendering, ordinary ground/falling/bouncing Morph Ball, Spring Ball ground/powered-jump/falling movement, normal-bomb placement/countdown/rendering/explosion/delete, and the morphed bomb-jump start/rise/handoff are translated. Bomb-triggered dynamic PLMs, live enemy damage producers, solid-enemy/liquid wall-jump branches, grapple spike-damage side effects, breakable grapple PLMs, equipped Speed Booster/shinespark state, native spawn selection, enemies, effects, and actors remain. Unsupported routes throw rather than becoming guessed physics. Raw files and PNGs contain private ROM-derived material and must not be distributed; this private preservation repository intentionally retains them until a future shareable cleanup.
