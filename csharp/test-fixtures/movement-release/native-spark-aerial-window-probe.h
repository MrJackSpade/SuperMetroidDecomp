...
// Airborne windup directional window: spin-jump first, Aim Up at frame 30,
// then apply forward after offsets 0..35. Compare launch choice and every frame.
int DiagnosticSparkAerialWindow(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"medium,left,offset,frame,input,pose,shine,windup,x,y\n");
  for(int medium=0;medium<3;medium++) for(int left=0;left<2;left++) for(int offset=0;offset<36;offset++) {
    ...
    for(int frame=0;frame<80;frame++) {
      ...
      if(frame==30) input|=0x10;
      if(frame>=30+offset) input|=left?0x200:0x100;
