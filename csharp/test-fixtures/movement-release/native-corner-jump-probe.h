...
// #457: corner jump reversal and delayed Jump windows at an upper ledge.
int DiagnosticCornerJump(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,speed,reverse,delay,jumpdelay,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,facing,yspeed,ydir,chargecounter,xradius,yradius\n");
  for (int left = 0; left < 2; left++)
  for (int speed = 0; speed < 3; speed++)
  for (int reverse = 0; reverse < 2; reverse++)
  for (int delay = 0; delay < 25; delay++)
  for (int jumpdelay = 0; jumpdelay < 9; jumpdelay++) {
    ...
    for (int frame = 0; frame < 112; frame++) {
      ...
      if (reverse && frame >= delay) direction = left ? 0x100 : 0x200;
      uint16 input = 0x8000 | direction;
      if (frame >= delay + jumpdelay && frame < delay + jumpdelay + 40) input |= 0x80;
      if (frame >= 80) input = 0;
      ...
      fprintf(f, "%d,%d,%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%02X,%04X%04X,%04X,%04X,%04X,%04X\n",
        left,speed,reverse,delay,jumpdelay,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,
