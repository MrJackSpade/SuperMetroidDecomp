...
// #456: ordinary damage during input-driven turn/morph/unmorph animations.
int DiagnosticKagoContact(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,pattern,hit,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,facing,yspeed,ydir,chargecounter,health,invincibility,hurt,direction\n");
  for (int left = 0; left < 2; left++)
  for (int pattern = 0; pattern < 3; pattern++)
  for (int hit = 6; hit <= 20; hit++) {
    ...
    samus_x_pos = samus_prev_x_pos = 1024; samus_y_pos = samus_prev_y_pos = pattern == 2 ? 249 : 235;
    ...
    for (int frame = 0; frame < 60; frame++) {
      ...
      uint16 input = 0;
      if (pattern == 0 && frame >= 8 && frame < 30) input = left ? 0x100 : 0x200;
      if (pattern == 1 && (frame == 8 || frame == 12)) input = 0x400;
      if (pattern == 2 && frame == 8) input = 0x800;
      if (frame == hit) {
        EnemyData *enemy = gEnemyData(0);
        enemy->enemy_ptr = 0xdcff;
        enemy->x_pos = samus_x_pos + (left ? -8 : 8);
        enemy->y_pos = samus_y_pos;
        cur_enemy_index = 0;
        ProbeRunBounded(0xa08023);
        enemy->enemy_ptr = 0;
      }
      ...
      fprintf(f, "%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%02X,%04X%04X,%04X,%04X,%04X,%04X,%04X,%04X\n",
        left,pattern,hit,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,
