...
// #463: spin breakout using Down+back, with forward-only control.
int DiagnosticDownbackDoor(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,speed,gap,delay,pattern,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,facing,yspeed,ydir,chargecounter,xradius,yradius,door\n");
  ...
  for (int pattern = 0; pattern < 2; pattern++) {
    ...
    for (int x=0;x<144;x++) level_data[30*144+x]=0x9000;
    ...
    samus_y_pos = samus_prev_y_pos = 461-gap;
    samus_y_dir = 2; samus_x_accel_mode = 2;
    samus_x_base_speed = 1; samus_x_base_subspeed = 0x4000;
    samus_x_extra_run_speed = speed * 2; samus_has_momentum_flag = 1;
    ...
    for (int frame = 0; frame < 24; frame++) {
      ...
      if (pattern && frame >= delay) direction = 0x400 | (left ? 0x100 : 0x200);
      ...
      fprintf(f, "%d,%d,%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%02X,%04X%04X,%04X,%04X,%04X,%04X,%04X\n",
        left,speed,gap,delay,pattern,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,
        ...
        samus_y_speed,samus_y_subspeed,samus_y_dir,flare_counter,movement_x_radius,movement_y_radius,door_def_ptr);
      if (door_def_ptr) break;
    }
  }
  fclose(f); return 0;
}
