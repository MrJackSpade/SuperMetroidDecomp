...
int DiagnosticPhantoonSpaced(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "start,cadence,movement,frame,input,frozen,health,timer,flash,map,x,y,samusX,samusY,pose,hud,samusHealth,function,samusSubX,samusSubY");
  ...
  fprintf(f, ",bodySubX,bodySubY,swoopVX,swoopVY,swoopTargetX,functionTimer,bodyInstruction,bodyInstructionTimer,eyeInstruction,eyeInstructionTimer,roundDamage,previousNewInput,cooldown\n");
  const int movements[] = {0,2,5,5,5};
  const int cadences[] = {10,10,9,10,11};
  for (int test = 0; test < 5; test++) {
    int start = 1640, cadence = cadences[test], movement = movements[test];
    ...
    equipped_items = collected_items = 1;
    ...
    samus_super_missiles = samus_max_super_missiles = 0;
    ...
    uint16 previous = 0;
    for (int frame = -2; frame < 1950; frame++) {
      nmi_frame_counter_word = nmi_frame_counter_byte = frame + 3;
      uint16 input = frame < 0 ? 0 : 0x800;
      if (frame >= 1460 && frame < 1480) input = 0x100;
      if (frame >= 1400 && frame < 1510 && frame % 12 == 0) input |= 0x40;
      if (frame == 1512) input |= 0x2000;
      if (frame >= 1530 && frame < 1570) input |= 0x80;
      if (frame == 1540 || frame == 1550) input |= 0x40;
      if (frame >= start) {
        int elapsed = frame - start;
        bool walk = movement == 2 && elapsed >= 60;
        if (movement == 5) walk = elapsed >= 60 && (elapsed - 60) % 10 < 2;
        input = elapsed == 0 || walk ? 0x200 : 0;
        if (elapsed == 0 || elapsed == 10 || (elapsed >= 60 && (elapsed - 60) % cadence == 0)) input |= 0x40;
      }
      ...
      fprintf(f, "%d,%d,%d,%d,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
        start,cadence,movement,frame,input,time_is_frozen_flag,body->health,body->invincibility_timer,
      ...
      fprintf(f, ",%u,%u,%u,%u,%u", body->x_subpos,body->y_subpos,
        Get_Phantoon(0x80)->phant_var_C,Get_Phantoon(0x80)->phant_var_D,Get_Phantoon(0x80)->phant_var_E);
      fprintf(f, ",%u,%u,%u,%u,%u,%u,%u,%u\n", Get_Phantoon(0)->phant_var_E,
        body->current_instruction, body->instruction_timer, gEnemyData(64)->current_instruction,
        gEnemyData(64)->instruction_timer, Get_Phantoon(128)->phant_var_B,
        joypad1_newinput_samusfilter, cooldown_timer);
      fflush(f);
    }
  }
  fclose(f); return 0;
