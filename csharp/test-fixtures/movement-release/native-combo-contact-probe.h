...
int DiagnosticComboContact(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  EnemyDef *def=get_EnemyDef_A2(0xf000); EnemyDef saved=*def;
  fprintf(f,"beam,blocking,overlap,slot,health,flash,inv,type,dir,pre,x,y,damage,next_count,next_types\n");
  const int beams[]={1,2,4,8};
  for(int b=0;b<4;b++) for(int blocking=0;blocking<2;blocking++)
  for(int overlap=0;overlap<2;overlap++) for(int p=0;p<4;p++) {
    ...
    fprintf(f,"%d,%d,%d,%d,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,",
      beams[b],blocking,overlap,p,e->health,e->flash_timer,e->invincibility_timer,
    ...
    ProbeRunBoundedRegisters(0x900000|projectile_bomb_pre_instructions[p],0,p*2,0);
