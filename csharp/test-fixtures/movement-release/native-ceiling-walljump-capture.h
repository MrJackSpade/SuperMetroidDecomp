// #445: full cartridge CPU trajectories for the ceiling-bonk continuous walljump.
// Forward stays held; missing-ceiling and adjacent timing/position cases are controls.
int DiagnosticCeilingWalljumpCapture(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,shift,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,yspeed,ydir,charge,spread,bombs,type,pointer0,x0,y0,vx0,vy0,bounce,jump,ceiling\n");
  for (int ceiling = 0; ceiling < 2; ceiling++)
  for (int left = 0; left < 2; left++) for (int jump = 132; jump <= 136; jump++)
  for (int shift = -6; shift <= 6; shift++) {
    ...
    for (int y = 0; y <= 16; y++) for (int x = 0; x < 144; x++)
      if (y == 16 || x == 1 || x == 142 || x == (left ? 58 : 85) && y >= 13 || ceiling && y == 7) level_data[y * 144 + x] = 0x8000;
    ...
    for (int frame = 0; frame < 180; frame++) {
      ...
      fprintf(f, "%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%04X%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%04X,%d,%d\n",
        left,shift,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,
        ...
        projectile_x_pos[5],projectile_y_pos[5],projectile_bomb_x_speed[5],projectile_bomb_y_speed[5],used_for_ball_bounce_on_landing,jump,ceiling);
    }
  }
  fclose(f); return 0;
}
