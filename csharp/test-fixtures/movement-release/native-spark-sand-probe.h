...
// Stored-shine attempts while immersed in surface/submerging Maridia sand.
// Suitless rejection runs through expiry; Gravity launches run through termination.
int DiagnosticSparkSand(const char *rom, const char *output) {
  ...
  for(int medium=0;medium<3;medium++) for(int left=0;left<2;left++) for(int offset=0;offset<4;offset++) {
    ...
    for(int y=13;y<16;y++) for(int x=1;x<15;x++) {
