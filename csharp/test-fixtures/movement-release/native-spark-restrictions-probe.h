...
// Stored-shine spin exits. Begin moving before Jump to avoid a grounded spark;
// at frame 30 keep Jump held and choose neutral, Shoot, Aim Up or Aim Down.
int DiagnosticSparkRestrictions(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"medium,left,offset,frame,input,pose,shine,windup,x,y\n");
  for(int medium=0;medium<3;medium++) for(int left=0;left<2;left++) for(int offset=0;offset<8;offset++) {
    ...
    equipped_items=0x2000|(medium==2?0x20:0); enable_horiz_slope_coll=3;
    ...
    for(int frame=0;frame<64;frame++) {
      ...
      // First launch a forward jump without consuming stored shine; then change aim.
      if(frame>=22 && frame<30) input|=left?0x200:0x100;
      if(frame>=30 && frame<36) { const uint16 aims[]={0x40,0x20,0x400,0x440}; input|=aims[offset/2]; }
      if(frame>=36 && (offset&1)) input|=0x10;
      ...
      fprintf(f,"%d,%d,%d,%d,%04X,%04X,%04X,%04X,%04X%04X,%04X%04X\n",
