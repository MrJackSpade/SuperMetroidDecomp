...
// Vertical/diagonal sparks cross water/lava/acid surface Y=128, Gravity off/on.
// Record liquid-state and acceleration handoff as well as fractional damage.
int DiagnosticSparkSurface(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"direction,left,liquidcase,frame,input,pose,shine,windup,x,y,health,crash,subhealth,liquid,accel\n");
  for(int medium=1;medium<3;medium++) for(int left=0;left<2;left++) for(int offset=0;offset<6;offset++) {
    ...
    fx_y_pos=offset<2?128:0xffff; lava_acid_y_pos=offset<2?0xffff:128;
    fx_liquid_options=0x80; fx_type=offset<2?6:offset<4?2:4; liquid_physics_type=offset<2?1:2;
    equipped_items=0x2000|((offset&1)?0x20:0); enable_horiz_slope_coll=3;
    ...
    for(int frame=0;frame<1200;frame++) {
      ...
      fprintf(f,"%d,%d,%d,%d,%04X,%04X,%04X,%04X,%04X%04X,%04X%04X,%04X,%d,%04X,%04X,%04X%04X\n",
        medium,left,offset,frame,input,samus_pose,samus_shine_timer,timer_for_shinesparks_startstop,
        samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,samus_health,samus_movement_handler==0xd346,samus_subunit_health,liquid_physics_type,samus_y_accel,samus_y_subaccel);
