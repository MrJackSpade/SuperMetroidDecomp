...
`--mother-brain-rainbow-script` runs the translated `$A9:B8EB-$BE95` repeat/active/final rainbow chain and its spawned Baby entrance. It advances actor AI before the separate ordinary enemy-instruction stage, reads Mother Brain's real walk and posture commands/spritemap delays from the supplied ROM, preserves all four 257-call waits and the power-bomb gate, drains Samus/resources for exactly 300 calls, narrows and shuts down the beam, executes the forced post-beam fall plus 129-call health decision, and retains the suit/RNG finish-off branches. The route reaches finish-off at 1047, waits for the forward body animation, copies four literal Baby graphics chunks through the live NMI queue on frames 1401..1404, verifies all `$800` VRAM bytes against the cartridge, requests the Baby spawn, and reaches Mother Brain's final firing hold at 1661. Baby initialization then waits the native 249 calls, reads the real `$A0:B443` signed sine table for its curve, calls drained-controller one, converges with the shared `$A9:F46B` 8.8 acceleration, requests Mother Brain's really-fast backward stumble at 1989, reaches the exact head pin at 1995 after `$A9:F5A6`'s predictive acceleration, and makes Mother Brain execute `$BE38` on 1996. The later corpse/drain/heal/death chain beginning at `$C8E2` remains explicit rather than guessed.
