...
int DiagnosticBombChainVariant(const char *rom, const char *output, int repeated) {
  ...
  bool ladder = repeated == 4;
  bool ceilingTravel = repeated == 5;
  ...
  for (int ceiling = ceilingTravel ? 1 : 0; ceiling < 2; ceiling++)
  for (int travel = 0; travel < (ceilingTravel ? 24 : horizontal ? 12 : triple ? 17 : 3); travel++)
  for (int schedule = 0; schedule < (ladder ? 5 : horizontal ? 13 : repeated == 1 ? 9 : 6); schedule++) {
    int spacing = ceilingTravel ? 1 + schedule : ladder ? 24 + schedule : horizontal ? 70 + 2 * schedule : triple ? 50 + schedule : repeated ? 48 + schedule : schedule ? 36 + 4 * schedule : 0;
    ...
    uint16 previous = 0;
    for (int frame = 0; frame < (ceilingTravel ? 480 : ladder ? 360 : repeated == 1 ? 600 : 180); frame++) {
      uint16 input = (ceilingTravel ? !frame || frame >= 52 && (frame - 52) % 24 == 0 : ladder ? !frame || frame >= 52 && (frame - 52) % spacing == 0 : horizontal ? !frame || frame == 52 || frame == spacing : triple ? !frame || frame == spacing || frame == 68 + travel : repeated ? frame % spacing == 0 : (!frame || (spacing && (frame == spacing || frame == 2 * spacing)))) ? 0x40 : 0;
      if (ceilingTravel && frame >= 170 + travel) {
        int phase = (frame - 170 - travel) % 24;
        if (!phase) input |= left ? 0x100 : 0x200;
        else if (phase <= spacing) input |= left ? 0x200 : 0x100;
      }
      if (horizontal && frame >= 74 && frame < 75 + travel) input |= left ? 0x200 : 0x100;
      if (ladder && frame >= 122 && frame < 126 && travel) input |= travel == 1 ? 0x200 : 0x100;
      if (!ceilingTravel && !ladder && !horizontal && !triple && frame >= 46 && frame < 50 && travel) input |= travel == 1 ? 0x200 : 0x100;
...
int DiagnosticLadderBombChains(const char *rom, const char *output) {
  return DiagnosticBombChainVariant(rom, output, 4);
}

int DiagnosticCeilingTravelBombChains(const char *rom, const char *output) {
  return DiagnosticBombChainVariant(rom, output, 5);
}
