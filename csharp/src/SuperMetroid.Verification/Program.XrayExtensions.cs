...
internal static partial class Program
{
    private static void VerifyXrayExtensions()
    {
        ...
        AssertEqual((ushort?)XrayRevealCodePointers.BlankMetatile, XrayRevealExtensions.Resolve(level, 18),
            "initial negative horizontal BTS reaches scroll-trigger blank");
        Set(level, 17, RoomCollisionType.ShootableBlock, 0);
        AssertTrue(XrayRevealExtensions.Resolve(level, 18) is null,
        ...
        AssertEqual((ushort?)XrayRevealCodePointers.BlankMetatile, XrayRevealExtensions.Resolve(level, 0),
            "negative coordinate exit emits native blank");
        Set(level, 0, RoomCollisionType.HorizontalExtension, 0);
        AssertTrue(XrayRevealExtensions.Resolve(level, 0) is null, "zero BTS retains original art");
        Set(level, 0, RoomCollisionType.HorizontalExtension, 1);
        Set(level, 1, RoomCollisionType.HorizontalExtension, 0xff);
        bool cycleRejected = false;
        try { XrayRevealExtensions.Resolve(level, 0); }
