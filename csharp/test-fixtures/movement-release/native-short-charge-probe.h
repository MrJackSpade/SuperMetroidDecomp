...
// #446: published multi-tap/stutter sequences and neighboring timing failures.
int DiagnosticShortCharge(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"left,taps,pattern,shift,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,boost,shine\n");
  for(int left=0;left<2;left++) for(int taps=2;taps<=4;taps++)
  for(int pattern=0;pattern<7;pattern++) for(int shift=-1;shift<=1;shift++) {
    ...
    for(int frame=0;frame<180;frame++) {
      ...
      // Forward-release frames encode none, 11_13, 9_14_, 5_7_10_,
      // 3_4_5_9_, an early release, and an overlong release.
      bool forward= !(pattern==1 && frame==11) &&
        !(pattern==2 && (frame==9 || frame==24)) &&
        !(pattern==3 && (frame==5 || frame==13 || frame==24)) &&
        !(pattern==4 && (frame==3 || frame==8 || frame==14 || frame==24)) &&
        !(pattern==5 && frame==1) && !(pattern==6 && frame>=11 && frame<=40);
      int magic[]={25,50,70,85}; bool dash=false;
      for(int tap=0;tap<taps;tap++)
        if(frame==magic[tap]+shift || (tap==taps-1 && frame>=magic[tap]+shift)) dash=true;
      uint16 input=store ? 0x8400 : (forward ? (left ? 0x200 : 0x100) : 0) | (dash ? 0x8000 : 0);
      ...
      fprintf(f,"%d,%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%04X,%04X\n",
        left,taps,pattern,shift,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,samus_pose,samus_movement_type,
