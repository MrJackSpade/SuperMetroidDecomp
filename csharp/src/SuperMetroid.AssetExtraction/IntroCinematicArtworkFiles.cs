...
public static class IntroCinematicArtworkFiles
{
    ...
    public static void Extract(ISnesAddressSpace bus, string directory, string sourceCartridgeSha256)
    {
        ...
        WriteDiscoveryActorSprites();
        WriteScientistSprites();
        WritePalette();
        ...
        void WriteRinkaSprites()
        {
            var frames = new Dictionary<string, SpriteVisualPart[]>(StringComparer.Ordinal);
            foreach (IntroRinkaSpriteFrameDefinition definition in IntroRinkaSpriteDefinitions.Frames)
                frames.Add(definition.Name, IntroCinematicSpriteFrameExtractor.Extract(bus,
                    definition.Pointer, IntroRinkaSpriteDefinitions.StockPartCount,
                    definition.Name));
            using var json = new MemoryStream();
            IntroRinkaSpritePresentation.Write(json, new IntroRinkaSpriteDocument
            {
                Version = IntroRinkaSpriteFormat.Version,
                Frames = frames,
            });
            byte[] encoded = json.ToArray();
        ...
        void WriteDiscoveryActorSprites()
        {
            var frames = new Dictionary<string, SpriteVisualPart[]>(StringComparer.Ordinal);
            foreach (IntroDiscoveryActorSpriteFrameDefinition definition in
                ...
                frames.Add(definition.Name, IntroCinematicSpriteFrameExtractor.Extract(bus,
                    definition.Pointer, definition.StockPartCount, definition.Name));
            using var json = new MemoryStream();
            IntroDiscoveryActorSpritePresentation.Write(json,
                new IntroDiscoveryActorSpriteDocument
                {
        ...
        void WriteScientistSprites()
        {
            ...
            foreach (IntroScientistSpriteFrameDefinition definition in
                ...
                frames.Add(definition.Name, IntroCinematicSpriteFrameExtractor.Extract(bus,
                    definition.Pointer, definition.StockPartCount, definition.Name));
            using var json = new MemoryStream();
            IntroScientistSpritePresentation.Write(json, new IntroScientistSpriteDocument
            {
                Version = IntroScientistSpriteFormat.Version,
                Frames = frames,
            });
            byte[] encoded = json.ToArray();
    ...
    public static IntroCinematicArtworkCatalog Load(string stockDirectory, string? overrideDirectory)
    {
        ...
        return new IntroCinematicArtworkCatalog(
            ...
            LoadDiscoveryActorSprites(),
            LoadScientistSprites(),
            LoadPalette(),
            LoadCeresFlight(),
        ...
        IntroRinkaSpritePresentation LoadRinkaSprites()
        {
            (string path, byte[] selected) = ReadSelected(IntroRinkaSpriteFormat.FileName);
            try
            {
                return IntroRinkaSpritePresentation.Load(
                    new MemoryStream(selected, writable: false));
            }
            catch (InvalidDataException error)
            {
                throw new InvalidDataException($"Invalid intro Rinka sprites {path}: {error.Message}",
                    error);
            }
        }
        ...
        IntroDiscoveryActorSpritePresentation LoadDiscoveryActorSprites()
        {
            (string path, byte[] selected) = ReadSelected(
                IntroDiscoveryActorSpriteFormat.FileName);
            try
            {
                return IntroDiscoveryActorSpritePresentation.Load(
                    new MemoryStream(selected, writable: false));
            }
            catch (InvalidDataException error)
            {
                throw new InvalidDataException(
                    $"Invalid intro discovery actor sprites {path}: {error.Message}", error);
            }
        }

        IntroScientistSpritePresentation LoadScientistSprites()
        {
            (string path, byte[] selected) = ReadSelected(IntroScientistSpriteFormat.FileName);
            try
            {
                return IntroScientistSpritePresentation.Load(
                    new MemoryStream(selected, writable: false));
            }
            catch (InvalidDataException error)
            {
                throw new InvalidDataException(
                    $"Invalid intro scientist sprite artwork {path}: {error.Message}", error);
            }
        }
    ...
    [
        ...
        IntroDiscoveryActorSpriteFormat.FileName,
        IntroScientistSpriteFormat.FileName,
        .. Enumerable.Range(0, IntroCinematicArtworkFormat.BackgroundPageCount)
