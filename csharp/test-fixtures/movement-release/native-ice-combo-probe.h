...
int DiagnosticIceCombo(const char *rom, const char *output) {
  ...
  for(int left=0;left<2;left++) for(int hit=-1;hit<=610;hit=hit<0?10:hit==10?610:611) {
    ...
    for(int frame=0;frame<640;frame++) {
      samus_x_pos=128+frame%9; samus_y_pos=128;
      ...
      for(int i=0;i<4;i++) {
        ...
        else fprintf(f,"%04X%04X%04X%04X%04X%04X%04X",projectile_x_pos[i],projectile_y_pos[i],projectile_bomb_pre_instructions[i],
          projectile_variables[i],projectile_bomb_x_speed[i],projectile_bomb_y_speed[i],projectile_timers[i]);
        fprintf(f,i==3?"\n":"/");
      }
    }
  }
