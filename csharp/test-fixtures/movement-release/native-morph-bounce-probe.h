...
int DiagnosticMorphBounce(const char *rom, const char *output) {
  ...
  for (int held = 0; held < 2; held++) {
    ...
    samus_movement_type = samus_prev_movement_type = samus_prev_movement_type2 = 8;
    samus_anim_frame_timer = 1; samus_x_speed_table_pointer = 0x9f55;
    samus_input_handler = 0xe913; samus_movement_handler = 0xa337;
    samus_y_speed = speeds[speed] >> 16; samus_y_subspeed = speeds[speed]; samus_y_dir = 2;
    uint32 base = carry < 4 ? carries[carry] : 0x14000;
    samus_x_base_speed = base >> 16; samus_x_base_subspeed = base;
    if (carry >= 4) {
      samus_x_extra_run_speed = carries[carry] >> 16;
      samus_x_extra_run_subspeed = carries[carry];
    }
