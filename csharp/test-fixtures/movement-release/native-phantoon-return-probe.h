...
int DiagnosticPhantoonReturn(const char *rom, const char *output) {
  ...
  const int movements[] = {21,22};
  const int cadences[] = {10,10};
  for (int test = 0; test < 2; test++) {
    ...
    for (int frame = -2; frame < 1850; frame++) {
      ...
      if (frame >= 1548 && frame < 1566 + (movement - 21) * 8) input |= 0x80;
      if (frame == 1565) input |= 0x40;
