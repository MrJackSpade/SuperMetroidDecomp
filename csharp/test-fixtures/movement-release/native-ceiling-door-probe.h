...
// #463: paired ceiling/no-ceiling movement at neighboring door and solid ceiling tiles.
int DiagnosticCeilingDoor(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,speed,ceiling,height,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,facing,yspeed,ydir,chargecounter,xradius,yradius,door\n");
  ...
  for (int height = 0; height < 9; height++) {
    ...
    level_data[28*144+(left ? 63 : 64)]=0x9000;
    if (ceiling) level_data[28*144+(left ? 64 : 63)]=0x8000;
    ...
    samus_y_pos = samus_prev_y_pos = 476 + height;
    samus_y_dir = 1; samus_y_speed = 4; samus_x_accel_mode = 2;
    ...
    for (int frame = 0; frame < 64; frame++) {
      ...
      fprintf(f, "%d,%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%02X,%04X,%04X,%04X%04X,%04X%04X,%04X,%02X,%04X%04X,%04X,%04X,%04X,%04X,%04X\n",
        left,speed,ceiling,height,frame,input,samus_x_pos,samus_x_subpos,samus_y_pos,samus_y_subpos,
