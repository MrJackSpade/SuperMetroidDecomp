...
Dash is a literal `$90:973E` route, not a viewer speed multiplier. Toggle **Speed Booster
equipped** if desired, then hold **Hold Run** with
...
jumps receive the exact extra vertical bonus. Palette copying, visible trailing echoes,
stored shine, and shinespark remain separate follow-up systems. Useful breakpoints are
`SamusHorizontalSpeedState.HandleExtraRunSpeed`, `SamusGroundedMovement.StepRunningRight`,
and `SamusState.AnimateNoFx`.
...
untranslated type-`$F` door. It verifies all five ROM-authored animation stages, stage-four
echo/contact state, exact 7.0000 cap, and the 7.C400 boosted spin-jump launch.
