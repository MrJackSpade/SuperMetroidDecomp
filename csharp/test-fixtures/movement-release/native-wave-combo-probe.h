...
int DiagnosticWaveCombo(const char *rom, const char *output) {
  int status=ProbeLoadRetailMovementRom(rom); if(status) return status;
  FILE *f=fopen(output,"wx"); if(!f) return 4;
  fprintf(f,"left,hit,frame,count,cooldown,flare,sounds,slots\n");
  for(int left=0;left<2;left++) for(int hit=-1;hit<=610;hit=hit<0?10:hit==10?610:611) {
    ...
    for(int frame=0;frame<640;frame++) {
      samus_x_pos=128+(left?-1:1)*(frame%9); samus_y_pos=128+(frame/7)%9;
