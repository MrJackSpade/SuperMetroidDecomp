...
// #463: complete spin-turn movement until first door trigger.
int DiagnosticMovingDoor(const char *rom, const char *output) {
  int status = ProbeLoadRetailMovementRom(rom); if (status) return status;
  FILE *f = fopen(output, "wx"); if (!f) return 4;
  fprintf(f, "left,speed,gap,delay,frame,input,x,y,pose,movement,anim,timer,base,extra,accel,facing,yspeed,ydir,chargecounter,xradius,yradius,door\n");
  ...
  for (int delay = 0; delay < 9; delay++) {
    ...
    level_data[29 * 144 + (left ? 61 : 66)] = 0x9000;
