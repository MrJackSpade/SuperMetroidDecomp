...
dotnet run --no-launch-profile --project src/SuperMetroid.DebugRunner -- "../Super Metroid.smc" --frames 1100 --mother-brain-rainbow-script --output ../standalone-assets/runtime/MotherBrainRainbowFrame.png
```
...
`--mother-brain-rainbow-script` runs the translated `$A9:B8EB-$BB2D` repeat/active rainbow cycle. It advances actor AI before the separate ordinary enemy-instruction stage, reads Mother Brain's real `$993A` walk commands and spritemap delays from the supplied ROM, preserves both 257-call charge timers and the power-bomb gate, drains Samus/resources for exactly 300 calls, narrows and shuts down the beam, and executes the forced post-beam fall plus 129-call health decision. The 1,100-frame route reaches the low-health finish-off seam at frame 1047; the later final-beam/Baby chain remains explicit rather than guessed.
