...
internal static class ZebetiteSkipComparison
{
    ...
    {
        if (frameCount is not (40 or 80 or 100 or 160 or 179 or 240 or 300))
            throw new ArgumentOutOfRangeException(nameof(frameCount), "Supported #442 intervals are 40, 80/160/179 (spark), 100, 240, or 300 frames.");
        ...
        for (int row = 1; row < managed.Length; row++)
        {
            ...
            if (actual.Length != fields.Length || expected.Length != fields.Length ||
                int.Parse(actual[0]) != (frameCount is 80 or 160 or 179 ? -1 : 119) + row || int.Parse(expected[0]) != (frameCount is 80 or 160 or 179 ? -1 : 119) + row)
