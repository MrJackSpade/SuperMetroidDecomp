...
int DiagnosticDamageBoost(const char *rom, const char *output) {
  ...
  if (!f) return 4;
  fprintf(f, "timer,ball,left,source,forward,delay,frame,input,x,y,pose,animation,hurtTimer,hurtDirection,ySpeed,yDirection,baseSpeed,extraSpeed,previousPose,previousMetadata,olderPose,olderMetadata\n");
  ...
  for (int delay = 0; delay < 12; delay++) {
    ...
    for (int frame = -1; frame < 30; frame++) {
      ...
      fprintf(f, "%d,%d,%d,%d,%d,%d,%d,%04X,%04X%04X,%04X%04X,%02X,%04X,%04X,%04X,%04X%04X,%04X,%04X%04X,%04X%04X,%04X,%04X,%04X,%04X\n",
        ...
        samus_prev_pose, *(uint16 *)&samus_prev_pose_x_dir, samus_last_different_pose, *(uint16 *)&samus_last_different_pose_x_dir);
    }
  }
  fclose(f); return 0;
}
